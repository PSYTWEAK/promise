...
contract("PromiseChef", async (accounts) => {
  ...
  it("Check pendingProm after 0 block", async () => {
    const pendingProm = await promiseChef.pendingProm(0, accounts[0]);
    assert.equal(pendingProm, 0);
  });
  it("Check pendingProm after 1 block", async () => {
    await timeTravel.advanceBlock();
    const pendingProm = await promiseChef.pendingProm(0, accounts[0]);
    assert.equal(pendingProm, TEST_DATA.DEPLOY.promPerBlock);
  });
});
