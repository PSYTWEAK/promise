...
contract("PromiseChef", async (accounts) => {
  ...
  it("Setting Promise Token / Test Token pool allocation points", async () => {
    await promiseChef.set(
      0,
      TEST_DATA.POOL_1.allocationPoints,
      TEST_DATA.POOL_1.withUpdate
    );
    const poolInfo = await promiseChef.poolInfo(0);
    assert.equal(poolInfo.allocPoint, TEST_DATA.POOL_1.allocationPoints);
  });
});
