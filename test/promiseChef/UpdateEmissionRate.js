...
contract("PromiseChef", async (accounts) => {
  ...
  it("Setting Prom per block", async () => {
    const newPromPerBlock = Math.round(TEST_DATA.DEPLOY.promPerBlock / 10);
    await promiseChef.updateEmmissionRate(newPromPerBlock);
    const promPerBlock = await promiseChef.promPerBlock();
    assert.equal(newPromPerBlock, promPerBlock);
  });
});
